...
function getPolynom() {
    if(arguments.length){
        ...
        return (x)=>{
            return arg.reduce((z,y,i)=>{
                return z + y * Math.pow(x, arg.length - i - 1)
            }, 0);
        };
    }else return null;
}
...
function memoize(func) {
    var cached = func.apply(this, arguments);
    return () => cached;
}
...
function logger(func, logFunc) {
    return function() {
        var str = JSON.stringify(Array.from(arguments));
        str = func.name +'('+str.slice(1,-1)+')';
        logFunc(str+" starts");
